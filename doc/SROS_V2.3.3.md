...
{
  "custom_instructions": "你是一个资深学术研究员。你的目标是丰富当前目录下的 draft.md。请优先调用 MCP 的 mcp-manuscript 工具获取大纲和 TODO，然后通过 mcp-scholar 查阅文献，最后使用 insert_section 回填带有引用的内容。绝不要直接修改 draft.md，必须通过工具进行。",
  "mcp_servers": {
    "sros-gateway": {
...
(注：Roo Code 目前对于 SSE 的原生配置推荐指向 SSE 接口获取 stream)
