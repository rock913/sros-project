...
### b. The MPA Development Loop (Multi-Agent Protocol Architecture)

Development in this project follows a strict separation of concerns, simulating a multi-agent team.

#### 1. Role: The Architect (GitHub Copilot)
- **Responsibility:** Design the "Contract" (Domain Layer).
- **Output:** Pydantic Schemas (`domain/schemas/`) and Protocols (`domain/ports/`).
- **Artifact:** A detailed `Session Plan` in `.ai-sessions/development/` and updated contract files.
- **Constraint:** NEVER writes implementation code.

#### 2. Role: The Builder (Aider)
- **Responsibility:** Implement the Contract (Infrastructure Layer).
- **Tooling:** Uses the CLI `aider` command generated by the Architect.
- **Rules:**
  - Must respect the `--read` context (Contracts are immutable during build).
  - Must use scoped linting (`ruff check <files>`) to avoid context explosion.
  - Must use `try-import` patterns for robust dependency handling.

#### 3. Role: The Inspector (Aider/Pytest)
- **Responsibility:** Verify the Implementation.
- **Output:** Green tests (`tests/`).
- **Strategy:** Patch locally (where used), not globally.

### c. Snapshot-Driven Development
