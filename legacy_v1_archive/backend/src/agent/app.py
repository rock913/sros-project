...
async def lifespan(app: FastAPI):
    """Initialize database and observability on startup."""
    ...
    print("--- Shutting down ---")
    if LangfuseManager.get_instance():
        LangfuseManager.get_instance().flush()
...
class AgentOutput(BaseModel):
    ...
    # Phase 5.3: Co-STORM Support
    mindmap: MindMap | None = Field(None, description="The generated research mindmap (Co-STORM).")
...
def get_agent_state_by_thread(
    ...
    **Fallback**: If checkpointer is unavailable (e.g., in tests), returns documents from database.
    ...
    try:
        ...
        # FIX: Directly access checkpointer to bypass graph schema validation
        # This allows reading Co-STORM (Async) state using the Sync checkpointer,
        # as long as they share the same DB.
        state_values = None
        
        # 1. Try fetching via get_state (preferred if schemas match)
        try:
        ...
        # 2. If get_state failed or returned empty (e.g. schema mismatch), try raw checkpointer access
        if not state_values:
            checkpoint = graph.checkpointer.get(config)
            if checkpoint and checkpoint.get("channel_values"):
                 print(f"[Fallback] Found checkpoint via raw checkpointer for thread {thread_id}")
                 state_values = checkpoint["channel_values"]
        
        # 3. Last Resort: Raw SQL Query
        if not state_values:
            print(f"[Fallback] Querying raw checkpoints table for thread {thread_id}")
            try:
                # Use sync_connection_pool to query directly
                with sync_connection_pool.connection() as conn:
                    with conn.cursor() as cur:
                        # LangGraph 0.2 table structure: thread_id, checkpoint_id, checkpoint, metadata
                        cur.execute(
                        ...
                        row = cur.fetchone()
                        if row:
                            print(f"[Fallback] Found checkpoint in RAW SQL for thread {thread_id}")
                            # checkpoint column is usually binary pickle or jsonb
                            # In PostgresSaver, it relies on LangChain serializer.
                            # We'll rely on the fact that if it's there, checkpointer.get SHOULD have found it.
                            # If we are here, something is weird. We'll verify existence at least.
                            # But we can try to manually load it if it's bytea.
                            # However, simply printing it exists is good diagnostic.
                            # Let's try to pass the raw dict if possible.
                            raw_checkpoint_data = row[0]
                            # If it's bytes, we might need to unpickle/json load.
                            # For now, let's just log existence.
                            pass
            except Exception as e:
                print(f"[Fallback] Raw SQL query failed: {e}")

        # If still no state found, return 404
        if not state_values:
            print(f"[Error] Thread {thread_id} not found in Graph, Checkpointer, or DB.")
            raise HTTPException(
                status_code=404,
        ...
        # Co-STORM Adapter: Map 'documents' to 'literature_abstracts' if needed
        literature_abstracts = state.get("literature_abstracts", [])
        if not literature_abstracts and "documents" in state:
             # Co-STORM 'documents' might be different structure
             documents = state.get("documents", {})
             if isinstance(documents, dict):
                 # Flatten dictionary of docs
                 for query, docs in documents.items():
                     if isinstance(docs, list):
                         literature_abstracts.extend(docs)

        # Co-STORM Adapter: MindMap
        mindmap = state.get("mindmap")

        return AgentOutput(
            messages=messages,
            research_topic=state.get("research_topic", "") or state.get("topic", ""), # Support 'topic' from Co-STORM
    ...
    try:
        ...
        if workflow_type == "costorm":
            ...
            input_data = {
                ...
                "session_id": session_id,  # CRITICAL: Pass session_id to Co-STORM graph
                "thread_id": thread_id,   # Pass thread_id for consistency
            ...
            if LangfuseManager.is_enabled():
                try:
                    ...
                    lf_handler = CallbackHandler(
                        ...
                        session_id=session_id,
                        tags=["websocket", workflow_type]
                    )
                    run_config["callbacks"] = [lf_handler]
                    print(f"[Observability] Attached Langfuse callback for session {session_id}")
                except Exception as e:
                    print(f"[Observability] Failed to attach Langfuse callback: {e}")

            # Use selected_graph.astream
            async for chunk in selected_graph.astream(input_data, config=run_config):
        ...
        # Determine paper count based on workflow type
        paper_count = len(result.get('literature_abstracts', []))
        if paper_count == 0 and 'documents' in result:
            # Co-STORM uses 'documents': Dict[node_id, List[Paper]]
            documents = result['documents']
            if isinstance(documents, dict):
                paper_count = sum(len(docs) for docs in documents.values() if isinstance(docs, list))
        ...
        if not result.get("stop_research"):
            await websocket.send_json({
                "type": "progress",
                "node": "agent_complete",
                "message": f"Research completed. Found {paper_count} papers."
            })
