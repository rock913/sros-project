...
exports.DraftWatcherProvider = void 0;
exports.createDraftWatcherProvider = createDraftWatcherProvider;
const vscode = require("vscode");
class DraftWatcherProvider {
     ...
     */
    async syncDocumentContent(document) {
        try {
            ...
            if (result.success) {
                ...
                if (gapCount > 0) {
                    ...
                    const highConfidenceGaps = this.activeGaps.filter((g) => g.is_high_confidence);
                    if (highConfidenceGaps.length > 0) {
                        vscode.window.showInformationMessage(`Found ${highConfidenceGaps.length} high-confidence evidence gaps. Click status bar to fix them.`);
                    }
                }
                else {
    ...
    async proposeEditsForGaps() {
        ...
        try {
            ...
            if (result.success && result.improvements?.length > 0) {
                ...
                vscode.window.showInformationMessage(`Generated ${result.improvements.length} improvement suggestions`);
            }
            else {
                this.statusBarItem.text = "$(warning) No edits generated";
                vscode.window.showWarningMessage(result.message || "No improvement suggestions generated");
            }
        }
        catch (error) {
     ...
     */
    showEditSuggestions(improvements) {
        ...
        const summary = improvements.map((imp, i) => `${i + 1}. ${imp.suggested_insertion.substring(0, 100)}... (${imp.citations?.length || 0} citations)`).join('\n');
        vscode.window.showInformationMessage(`AI Edit Suggestions Ready:\n${summary}`, "Apply All").then(selection => {
     ...
     */
    async applyAllEdits(improvements) {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
            ...
            await editor.edit(editBuilder => {
                for (const improvement of improvements) {
                    ...
                    if (snippetIndex !== -1) {
                        ...
                        const endPos = editor.document.positionAt(snippetIndex + improvement.original_snippet.length);
                        // Apply the suggested edit
                        editBuilder.replace(new vscode.Range(startPos, endPos), improvement.suggested_insertion);
                    }
                }
            });
     ...
     */
    async acceptEditSuggestion(editSpec) {
            ...
            if (result.success && result.edit_spec) {
                ...
                if (editor && result.edit_spec) {
                    ...
                    await editor.edit(editBuilder => {
                        ...
                        editBuilder.replace(new vscode.Range(startPos, endPos), edit.new_text);
                    });
...
exports.DraftWatcherProvider = DraftWatcherProvider;
// Export factory function for extension activation
function createDraftWatcherProvider(mcpClient) {
    return new DraftWatcherProvider(mcpClient);
}
//# sourceMappingURL=DraftWatcher.js.map