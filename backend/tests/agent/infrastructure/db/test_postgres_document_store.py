...
class TestPostgresDocumentStore(unittest.TestCase):
    ...
    def test_save_document(self, mock_get_db_connection):
        # Arrange
        # Mock the Context Manager pattern: with get_db_connection() as session:
        mock_session = MagicMock()
        mock_get_db_connection.return_value.__enter__.return_value = mock_session
        ...
        # Assert
        # The session is obtained via __enter__, so we check calls on that mock
    ...
    def test_exists(self, mock_get_db_connection):
        ...
        # Assert
        mock_session.query.assert_called_once_with(DbDocument)
        # We don't necessarily call scalar() in implementation, logic check:
        # return session.query(DbDocument).filter(DbDocument.source == source_id).first() is not None
        # So check call sequence on query result
        # mock_session.query().filter().first()
