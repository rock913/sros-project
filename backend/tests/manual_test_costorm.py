...
async def test_costorm_graph_isolation():
    ...
    try:
        ...
        if hasattr(result_state, 'mindmap') and result_state.mindmap:
            print(f"  - MindMap: ✓ Generated ({len(result_state.mindmap.nodes)} perspectives)")
            for i, node in enumerate(result_state.mindmap.nodes):
                print(f"    [{i+1}] {node.name[:40]}... ({node.id})")
        else:
            print("  - MindMap: ✗ Not generated or None")
            return False

        if hasattr(result_state, 'perspectives') and result_state.perspectives:
            print(f"  - Perspectives: ✓ Generated ({len(result_state.perspectives)} items)")
        else:
