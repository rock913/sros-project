...
async def check_server_health(port):
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            # 1. Check SSE Endpoint existence
            print(f"[1] Pinging SSE endpoint...")
            # We use a stream context to just open the connection and check status, then close
            async with client.stream("GET", f"{base_url}/sse", headers={"Accept": "text/event-stream"}) as resp:
                if resp.status_code == 200:
                    print(f"    ✅ SSE Endpoint is UP (Status 200)")
                else:
                    print(f"    ❌ SSE Endpoint returned {resp.status_code}")
                    return

            # 2. Simulate Initialization (JSON-RPC)
            print(f"[2] Sending JSON-RPC initialize...")
            payload = {
                "jsonrpc": "2.0",
                "method": "initialize",
                "params": {
            ...
            print(f"    ❌ Read Timeout (Server hung?)")
