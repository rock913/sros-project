    ...
    try:
        async with sse_client(sse_url) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                # 2. Test Manuscript Manager
                logger.info("🧪 Testing Manuscript Manager (manuscript_get_structure)...")
                try:
                    result = await session.call_tool("manuscript_get_structure", {})
                    # Parse text content from the first TextContent object
                    content = result.content[0].text
                    structure = json.loads(content)
                    if "headers" in structure:
                         logger.info(f"✅ Manuscript structure retrieved. Found {len(structure['headers'])} headers.")
                    else:
                         logger.warning(f"⚠️ Unexpected structure format: {content[:100]}...")
                except Exception as e:
                    logger.error(f"❌ Manuscript test failed: {e}")

                # 3. Test Federal Search (Quick Check)
                logger.info("🧪 Testing Federal Search (federal_search_papers)...")
                try:
                    # Searching for something specific but rare to avoid huge payloads
                    search_res = await session.call_tool("federal_search_papers", {
                        "query": "SROS: Scholarly Research Operating System", 
                        "limit": 1
                    })
